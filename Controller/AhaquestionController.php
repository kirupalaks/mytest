...
class AhaquestionController extends AppController {
	...
	public function adata_add_slide() {
	    $result;
	    $data = $this->request->data; //echo $data['File']['question']['name'];die;
		if($data['question_type'] == 1) {
			$mcqtext = str_replace('   ', '', $data['question']['text']);	
			$now = date('Y-m-d-His');
			if (!empty($data['File']['question']['name'])) 
			{
				move_uploaded_file($data['File']['question']['tmp_name'], "mobileapp_content/" .$now.$data['File']['question']['name']);
				$qimage = $now.str_replace(' ', '_',$data['File']['question']['name']);
			}
			else $qimage = "";
			if (!empty($data['File']['answer']['name'])) 
			{
				move_uploaded_file($data['File']['answer']['tmp_name'], "mobileapp_content/" .$now.$data['File']['answer']['name']);
				$ansimage = $now.str_replace(' ', '_',$data['File']['answer']['name']);
			}
			else $ansimage = "";
			if (!empty($data['File']['solution']['name'])) 
			{
				move_uploaded_file($data['File']['question']['tmp_name'], "mobileapp_content/" .$now.$data['File']['solution']['name']);
				$solimage = $now.str_replace(' ', '_',$data['File']['solution']['name']);
			}				
			else $solimage = "";	
			$coreqtxt = array('text' => $mcqtext,'image'=>$qimage,'mark'=>$data['question']['mark'],'correct_answer'=>$data['question']['correct_answer']);
		...
		$question_info = array('name'=>$data['name'],'questiontext'=>$coreqtxt,'answer'=>array('image'=>$ansimage),
		'option' => $options,
		'solution' => array('text'=>$data['question']['solution_text'],'image'=>$solimage,'video'=>$data['question']['solution_video'])			
		);
	...
	public function adata_edit_slide() {
		...
		if($data['question_type'] == 1) {
			...
			{
				move_uploaded_file($data['File']['question']['tmp_name'], "mobileapp_content/" .$now.$data['File']['question']['name']); 
				$questionimage = $now.$data['File']['question']['name'];	
			...
			{
				move_uploaded_file($data['File']['answer']['tmp_name'], "mobileapp_content/" .$now.$data['File']['answer']['name']); 
				$answerimage = $now.$data['File']['answer']['name'];	
			} else if (!empty($data['oldFile']['answer']) )  { $answerimage = $data['oldFile']['answer']; }
			if (!empty($data['File']['solution']['name'])) 
			{
				move_uploaded_file($data['File']['solution']['tmp_name'], "mobileapp_content/" .$now.$data['File']['solution']['name']); 
				$solutionimage = $now.$data['File']['solution']['name'];	
			} else if (!empty($data['oldFile']['solution']	)) { $solutionimage = $data['oldFile']['solution']; }
			$data['complexity'] = $data['question']['diff_level'];
			if($data['complexity']== 'easy') {$mark = 2;} else if($data['complexity']== 'medium') { $mark=3; }
			else if($data['complexity']== 'hard') {$mark=4;}
			$mcqtext = $data['question']['text'];				
			$coreqtxt = array('text' => $mcqtext,'image'=>$questionimage,'mark'=>$mark,'correct_answer'=>$data['question']['correct_answer']);
