<?php
	...
	$secret_key = "e8ea4cd44d1e3bf222058592fcc464d7";	 // Your Secret Key

	$response = array();
	//require('Rc43.php');
	$getparams=split("DR=", $_SERVER['QUERY_STRING']);
	error_log("AAA response.php ".$getparams[1]);
if(isset($getparams[1])) {
         error_log("AAA DR is set");
	 //require('Rc43.php');
	 $DR = preg_replace("/\s/","+",$getparams[1]);
...
<center>
   <table width='100%' cellpadding='0' cellspacing="0" ><tr><th width='90%'><h2 class='co'>&nbsp;EBS Payment Integration Page - Version 2</h2></th></tr></table>
	...
	<form method="post" action="/student/puzzler">
